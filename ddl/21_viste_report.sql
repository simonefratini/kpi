...
select p.name as project,
       ...
       c.wbs as wbs,
       c.lob as lob,
       ifnull(concat(`u`.`lastname`, ' ', `u`.`firstname`),'not set') as `project_manager`,
       c.project_alias,
       c.site as site
  ...
  left join (select x.project_id as project_id,
                    ...
                    max((case when (x.id = 36) then x.value else null end)) as project_manager_id,
 ...
 left join redmine.users u on u.id=c.project_manager_id;
