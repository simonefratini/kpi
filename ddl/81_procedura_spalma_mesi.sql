...
DELIMITER $$
CREATE PROCEDURE insert_spalmato_sui_mesi(p_id int, p_user_id int, p_inizio datetime, p_fine datetime )
BEGIN
...
while inizio_mese < ifnull(p_fine,now()) do
	...
	insert into tmp_team_performance (id, user_id, aperto, chiuso) values (p_id,p_user_id, inizio_mese, fine_mese);
